(function () {
    ...
    els.downloadAllBtn.addEventListener('click', async () => {
        const doneItems = queue.items.filter((it) => it.status === LINE_STATUS.DONE && it.audioBlob);

        if (!doneItems.length) {
            alert('Chưa có file nào hoàn thành để tải. Các dòng đang "chờ" hoặc "lỗi" sẽ tự động bị bỏ qua.');
            return;
        }

        if (typeof JSZip === 'undefined') {
            alert('Không tìm thấy thư viện nén (lib/jszip/jszip.min.js). Kiểm tra lại file đã được tải đủ chưa.');
            return;
        }
        ...
        try {
            ...
            doneItems.forEach((item) => {
                ...
                const lineNumber = queue.items.indexOf(item) + 1;
                const safeName = `dong_${String(lineNumber).padStart(3, '0')}.${ext}`;
                folder.file(safeName, item.audioBlob);
            });

            els.downloadAllBtn.textContent = `Đang nén (0/${doneItems.length})...`;
            const zipBlob = await zip.generateAsync({ type: 'blob' }, (metadata) => {
                els.downloadAllBtn.textContent = `Đang nén (${Math.round(metadata.percent)}%)...`;
            });
            ...
            alert('Lỗi khi nén file: ' + (err.message || err));
        } finally {
